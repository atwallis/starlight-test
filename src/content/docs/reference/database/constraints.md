...
```sql
"createdOn" >= now() - INTERVAL '1 year'
```

Ensures records are not older than one year.
...
OR (
  "expiryYear" = EXTRACT(YEAR FROM now()) 
  AND "expiryMonth" >= EXTRACT(MONTH FROM now())
)
```

Ensures credit card is not expired.
...
-   [Set up relationships with foreign keys](/starlight-test/reference/database/relationships/)
