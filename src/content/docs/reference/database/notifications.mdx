...
eventManager.addRowInsertHandler<User>(
	async (data, queryMetadata) => {
		console.log('New user created:', data.record);
		console.log('User ID:', data.insertedId);
		
		// Send welcome email
		await sendWelcomeEmail(data.record.email);
...
eventManager.addColumnChangeHandler<Order>(
	async (data, queryMetadata) => {
		...
		console.log('Previous status:', data.previousRecord.status);
		
		// Send notification to customer
		if (data.record.status === 'SHIPPED') {
...
eventManager.addRowDeleteHandler<User>(
	async (data, queryMetadata) => {
		...
		console.log('Previous data:', data.previousRecord);
		...
		await auditLog.record('user_deleted', {
			...
			email: data.previousRecord.email,
...
eventManager.addColumnChangeHandler<Order>(
	async (data, queryMetadata) => {
		// data.record only contains: id, status, total, modifiedOn
		// Other columns are not included
		
		if (data.record.status !== data.previousRecord.status) {
			console.log('Status changed:', data.previousRecord.status, '→', data.record.status);
		}
		
		if (data.record.total !== data.previousRecord.total) {
			console.log('Total changed:', data.previousRecord.total, '→', data.record.total);
		}
	},
...
eventManager.addRowUpdateHandler<User>(
	async (data, queryMetadata) => {
		await auditLog.record('user_updated', {
			...
			changes: compareObjects(data.previousRecord, data.record),
...
eventManager.addColumnChangeHandler<Order>(
	async (data) => {
		if (data.record.status === 'COMPLETED' && data.previousRecord.status !== 'COMPLETED') {
...
eventManager.addRowInsertHandler<Order>(
	async (data, queryMetadata) => {
		...
		try {
			await processOrder(data.record);
		} finally {
...
-   [Set up relationships](/starlight-test/reference/database/relationships/)
