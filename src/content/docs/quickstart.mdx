...
<Aside type="note">
The authentication handler is called for every request. It should validate the user's credentials (JWT token, session, API key, etc.) and call `onValid()` with the user's role and scopes. For now, we're allowing all requests as the 'user' role. Learn more about [authentication in Restura](../concepts/authentication).
</Aside>
...
<Code code="touch restura.schema.json" lang="bash"/>
...
Learn more about the [schema structure](../reference/restura-schema).
</Aside>
...
<Code code="touch restura.config.mjs" lang="bash"/>
...
<Aside type="tip">
Since your schema has no database tables or endpoints yet, Restura won't register any routes. That's normal! Continue to the next steps to add your first table and endpoint.
</Aside>
