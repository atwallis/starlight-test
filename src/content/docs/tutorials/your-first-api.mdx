...
<Aside type="tip">
**Already have Express and PostgreSQL?** Follow the [QuickStart](../../quickstart) guide and then skip to [Step 7: Setup the Visual Editor](#step-7-setup-the-visual-editor).
</Aside>
...
<Aside type="note">
Bun includes TypeScript support by default, so you only need to install type definitions.
</Aside>
...
<Aside type="note">
The authentication handler is called for every request. For this tutorial, we're allowing all requests as the 'user' role. In production, you'd validate JWT tokens, API keys, or sessions here. Learn more about [authentication in Restura](../concepts/authentication).
</Aside>
...
<Aside type="tip">
For TypeScript, you may need to install `tsx`: `npm install -D tsx` and change the script to `"start": "tsx server.ts"`
</Aside>
...
<Aside type="note">
The `authToken` is important! The Visual Editor will use this token to authenticate with your Restura API. You can change it to any value you want, just remember it for later.
</Aside>
...
<Aside type="note">
Follow the **[Visual Editor Setup Guide](../../visual-editor)** to clone, install, and start the Visual Editor. Then come back here to continue the tutorial.
</Aside>
...
<Aside type="note">
  `id`, `createdOn` and `modifiedOn` are auto-created by Restura.
</Aside>
...
2. Click **"+ Add Index"**
   ...
   - Unique: ✓
3. Click **"+ Add Index"** again
   - Name: `user_username_unique_index`
   - Columns: Select `username`
   - Unique: ✓
4. Click **"Save"** to save the user table
...
<Aside type="note">
The Visual Editor only updates the `restura.schema.json` file. To actually create the table in PostgreSQL, you need to apply the schema to your database. The Visual Editor can generate the SQL for you!
</Aside>
...
<Aside type="tip">
The Visual Editor has a shortcut: you can click **"Add All Columns"** to automatically add all fields from the selected table!
</Aside>
...
<Aside type="tip">
**This is the power of Restura**: You design visually, and Restura generates the configuration, SQL, TypeScript types, and API endpoints automatically.
</Aside>
...
- ✅ Ran the Visual Editor to your API
