...
Build a GET endpoint that fetches data from a table, filters by a request parameter, and joins to a related table.

## What You'll Build

A read endpoint that:

-   Accepts a filter parameter from the request
-   Queries a base table with an optional join
-   Returns selected fields from one or more tables

<Aside type="tip" title="Running Example">
Throughout this guide, we'll show a concrete example: fetching a user by first name with their company info. Adapt the values to your own tables.
</Aside>
...
Click **Endpoints** in the sidebar to open the endpoint editor.
...
| Field           | What to Enter                                            |
| --------------- | -------------------------------------------------------- |
| **Type**        | `ONE` for a single record, `MANY` for a list             |
...
<Aside>
**Example:** For a user lookup, you might use:
...
<Aside>
**Example:** Add a `firstName` parameter with `string` validation to filter users by name.
</Aside>
...
1. Select **Standard Join**
2. Choose the **local column** — the foreign key in your base table
3. Choose the **foreign table** — the related table to join
4. Choose the **foreign column** — typically the primary key (`id`)
5. Click **Select**
...
<Aside type="caution">
Use `LEFT` join if the relationship is optional. For example, if users might not have a company, use `LEFT` to avoid excluding those users from results.
</Aside>
...
| -------- | --------------------------------- |
| `=`      | Exact match                       |
| `LIKE`   | Partial match (use `%` wildcards) |
| `>`/`<`  | Numeric or date comparisons       |
| `IN`     | Match any value in a list         |

<Aside type="tip">
Use `$paramName` syntax to reference request parameters. For example, `$firstName` references the `firstName` parameter from Step 5.
</Aside>
...
```
GET /v1/{your-path}?{parameter}={value}
```

<Aside>
**Example request:**
```
GET /v1/user/by-name?firstName=John
```
...
You created a standard endpoint with:

-   ✅ Request parameters with validation
-   ✅ A join to include related data (optional)
-   ✅ A where clause to filter results
-   ✅ A custom response shape

For more configuration options, see the [Endpoints Reference](../../reference/endpoints).
