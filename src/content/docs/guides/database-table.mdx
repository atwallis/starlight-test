...
<Aside type="note">
This guide assumes you've completed the [Getting Started](../../getting-started) tutorial and have the [Visual Editor](../../visual-editor) running.
</Aside>
...
1. Click the **pencil icon** (✏️) next to the table name
...
<Aside type="tip">
Name columns ending with `Id` (e.g., `userId`) to auto-create foreign keys.
</Aside>
...
<Aside type="tip">
Use the **Unique** toggle in the column row for single-column unique indexes.
</Aside>
...
For detailed information on column types, smart detection, and all available options, see the [Database Reference](../../reference/database).
